...
def open_db(LCS=None):
    ...
    con.execute("CREATE TABLE IF NOT EXISTS Music(Id INTEGER PRIMARY KEY, " +
                ...
                # Change ReleaseDate to FirstDate and RecordingDate to AlbumDate
                # For old iTunes stuff initialize AlbumDate with FirstDate.
    ...
    ''' music id's are reset starting at #1 for new database '''
    #populate_new_database()  # Get from July 13, 2023 backup.
    #convert_to_database3()  # New conversion one-time use
    #close_new_db()
    #exit()


def convert_to_database3():
    """ Read database formate 2 and create database format 3 """
    ...
    Revise: update_metadata2() below
        ...
        ''' (OsFileName, OsAccessTime, OsModifyTime, OsChangeTime,  
             OsFileSize, Title, Artist, Album, Compilation, AlbumArtist, 
        ...
        if live_update:
            ...
            sql = "INSERT INTO Music \
                   ...
                   Rating, Genre, Composer, Comment, Hyperlink, Duration, Seconds, \
                   GaplessPlayback, PlayCount, LastPlayTime, LyricsScore, \
                   LyricsTimeIndex) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, \
                                        ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
            ''' Keep same order for maintainability
                    "OsFileName TEXT, OsAccessTime FLOAT, OsModifyTime FLOAT, " +
                    "OsChangeTime FLOAT, OsFileSize INT, " +
                    "Title TEXT, Artist TEXT, Album TEXT, Compilation TEXT, " +
                    "AlbumArtist TEXT, AlbumDate TEXT, FirstDate TEXT, " +
...
def populate_new_location():
    ...
    for sql_row in rows:
        ...
        HostTestRep = row['HostTestRep']
        HostMountCmd = row['HostMountCmd']
        HostTouchCmd = row['HostMountCmd']
        HostTouchMinutes = row['HostMountMinutes']
        Comments = row['Comments']
        sql = "INSERT INTO History (Code, Name, ModifyTime, ImagePath, \
               MountPoint, TopDir, HostName, HostWakeupCmd, HostTestCmd, \
               HostTestRep, HostMountCmd, HostTouchCmd, HostTouchMinutes, \
               Comments) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
        new_loc_cursor.execute(
            sql,
            (Code, Name, ModifyTime, ImagePath, MountPoint, TopDir,
             HostName, HostWakeupCmd, HostTestCmd, HostTestRep, HostMountCmd, 
...
def open_new_db():
    ...
    new_hist_cursor = con.cursor()
    new_loc_cursor = con.cursor()
...
def update_metadata2(fc, commit=True):
    ...
    if d['Artist'] is None:
        ...
        # SQL adding metadata for: Filter/The Very Best Things: 1995–2008/01 Hey Man Nice Shot.oga
        ''' Keep same order for maintainability
    sql = "UPDATE Music SET Title=?, Artist=?, Album=?, Compilation=?, \
    ...
    '''
        Title TEXT, Artist TEXT, Album TEXT, Compilation TEXT,
        AlbumArtist TEXT, AlbumDate TEXT, FirstDate TEXT, DiscNumber TEXT, 
        TrackNumber TEXT, Genre TEXT, Composer TEXT, Comment TEXT, Duration TEXT, 
        Seconds FLOAT, GaplessPlayback TEXT    
    '''
    ...
    sql = "UPDATE Music SET Title=?, Artist=?, Album=?, Compilation=?, \
           ...
           Seconds=?, GaplessPlayback=? WHERE OsFileName = ?"
    cursor.execute(
        sql, (fc.Title, fc.Artist, fc.Album, fc.Compilation,
