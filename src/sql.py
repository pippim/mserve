...
def populate_new_database():
    """ Read old database to create new one """
    ...
    for sql_row in rows:
        ...
        sql = "INSERT INTO Music (OsFileName, OsAccessTime, OsModifyTime, \
               OsChangeTime, OsFileSize, Title, Artist, Album, ReleaseDate, \
               TrackNumber, Duration, Seconds, LyricsScore, LyricsTimeIndex) \
               VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
...
def open_new_db():
    ...
    ''' Adding  DiscNumber, CreationTime, Composer, Comment, Hyperlink (all TEXT)
        ...
        Change  OsModificationTime      -> OsModifyTime
                ...
                OriginalDate (FLOAT)    -> RemasterDate (TEXT)
        ...
        Review mserve FileControl() where metadata is scanned
    ...
    new_con.execute("create table IF NOT EXISTS Music(Id INTEGER PRIMARY KEY, " +
                    ...
                    "ReleaseDate TEXT, RemasterDate TEXT, " +
                    "CreationTime TEXT, DiscNumber TEXT, TrackNumber TEXT, " +
                    "Rating TEXT, Genre TEXT, Duration TEXT, " +
                    "Composer TEXT, Comment TEXT, Hyperlink TEXT, " +
...
def update_metadata(key, artist, album, title, genre, tracknumber, date, 
    ...
    if date:
        if type(date) is str:
            ...
            print("sql.update_metadata() invalid date")
    ...
    sql = "UPDATE Music SET Artist=?, Album=?, Title=?, Genre=?, TrackNumber=?, \
           ReleaseDate=?, Seconds=?, Duration=?, DiskNumber=?, Composer=? \
           WHERE OsFile = ?" 
...
def music_treeview():
    ...
    music_treeview_list = [
      ...
      OrderedDict([
        ...
        # convert July 13, 2023
        # ("var_name", "RemasterDate"), ("select_order", 0), ("unselect_order", 11),
        # ("key", False), ("anchor", "w"), ("instance", str), ("format", None),
    ...
    ''' convert July 13, 2023  INSERT ABOVE
      OrderedDict([
      ...
      OrderedDict([
        ...
        ("var_name", "DiskNumber"), ("select_order", 0), ("unselect_order", 22),
        ("key", False), ("anchor", "center"), ("instance", str), ("format", None),
        ("display_width", 160), ("display_min_width", 140),
        ("display_long", None), ("stretch", 1)]),
