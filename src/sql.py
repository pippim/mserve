...
def open_db():
    ...
    #populate_new_database()  # Old conversion can't use anymore
    #close_new_db()
    #exit()
...
def populate_new_database():
    ...
    for sql_row in rows:
        ...
        if live_update:
            sql = "INSERT INTO Music (OsFileName, OsAccessTime, OsModifyTime, \
                   ...
                   TrackNumber, Genre, Duration, Seconds, LyricsScore, LyricsTimeIndex) \
                   VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"

            new_cursor.execute(
                sql,
                (OsFileName, OsAccessTime, OsModifyTime, OsChangeTime,
                 OsFileSize, Title, Artist, Album, ReleaseDate, TrackNumber, Genre,
                 Duration, Seconds, LyricsScore, LyricsTimeIndex))  # new cursor
    ...
    # Below was just curiosity because total intended 2 spaces
    # print("ext.TIME_LIST:", ext.TIME_LIST, "ext.TIME_NDX:", ext.TIME_NDX)
    # ext.TIME_LIST: [('sql.create_tables()', 1689352289.438806)] ext.TIME_NDX: 1
